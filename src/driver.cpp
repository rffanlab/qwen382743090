...
bool NvidiaDriver::run_q6k_dequant_smoke(
    const std::byte* block,
    std::string* error,
    double* max_abs_error,
    double* max_rel_error,
    std::uint32_t* first_bad_index) {
    try {
        if (!available()) {
        ...
        if (!is_sm86()) {
            throw std::runtime_error(
                "q38 Q6_K dequant smoke requires sm_86; detected sm_" +
                std::to_string(sm_major_) +
                std::to_string(sm_minor_));
        }
        if (!block) {
            throw std::invalid_argument("Q6_K block is null");
        }

        std::array<float, kQ4KValuesPerBlock> ref{};
        std::array<float, kQ4KValuesPerBlock> gpu{};
        dequantize_q6_k_block_cpu(block, ref);

        constexpr std::size_t block_off = 0;
        constexpr std::size_t out_off = 256;
        const std::size_t total_bytes =
            out_off + gpu.size() * sizeof(float);
        ScopedVmm memory(
        ...
        const CUdeviceptr block_ptr =
            memory.ptr() + block_off;
        const CUdeviceptr out_ptr =
            memory.ptr() + out_off;

        check(handle_, memcpy_htod(
            block_ptr, block, kQ6KBytesPerBlock),
            "cuMemcpyHtoD(Q6_K dequant block)");
        ...
        if (rc != CUDA_SUCCESS) {
            std::string detail =
                cuda_error(
                    handle_, rc,
                    "cuModuleLoadDataEx(Q6_K dequant)");
            if (jit_error[0] != '\0') {
                detail +=
                    std::string("\nPTX JIT error log:\n") +
                    jit_error.data();
        ...
        try {
            ...
            check(handle_, module_get_function(
                &fn, module, "q38_dequant_q6k_block"),
                "cuModuleGetFunction(q38_dequant_q6k_block)");
            ...
            check(handle_, launch(
                ...
                "cuLaunchKernel(q38_dequant_q6k_block)");
            check(handle_, sync(),
                  "cuCtxSynchronize(Q6_K dequant)");
            check(handle_, memcpy_dtoh(
                gpu.data(),
                out_ptr,
                gpu.size() * sizeof(float)),
                "cuMemcpyDtoH(Q6_K dequant)");
        } catch (...) {
            module_unload(module);
            throw;
        }
        ...
        check(handle_, module_unload(module),
              "cuModuleUnload(Q6_K dequant)");

        double abs_max = 0.0;
        double rel_max = 0.0;
        std::uint32_t bad_index =
            static_cast<std::uint32_t>(gpu.size());

        for (std::size_t i = 0; i < gpu.size(); ++i) {
            const double got =
                static_cast<double>(gpu[i]);
        ...
        if (bad_index != gpu.size()) {
            std::ostringstream oss;
            oss << "Q6_K dequant mismatch: first_bad="
                << bad_index
                << " gpu=" << gpu[bad_index]
                << " cpu=" << ref[bad_index]
                << " max_abs=" << abs_max
