...
int main(int argc, char** argv) {
    std::string model;
    std::string tensor_name = "blk.0.ssm_beta.weight";
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--model" && i + 1 < argc) model = argv[++i];
            ...
            std::cout << "usage: q38-q4k-gemv --model MODEL.q38pack [--tensor NAME]\n";
            return 0;
        } else {
    ...
    if (model.empty()) {
        std::cerr << "q38-q4k-gemv: --model is required\n";
        return 2;
    }
    ...
    try {
        ...
        std::cout << "Q38RT fused Q4_K GEMV\n";
        ...
        if (!driver.run_q4k_gemv_smoke(
                ...
                &ms,
                &gbps)) {
            std::cerr << "q4_k_gemv: FAILED: " << error << "\n";
            return 6;
        }
        ...
        std::cout << "q4_k_gemv: PASS\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "q38-q4k-gemv: " << e.what() << "\n";
        return 1;
    }
}
