...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--model" && i + 1 < argc) model = argv[++i];
            ...
            std::cout << "usage: q38-q4k-smoke --model MODEL.q38pack\n";
            return 0;
        } else {
    ...
    if (model.empty()) {
        std::cerr << "q38-q4k-smoke: --model is required\n";
        return 2;
    }
    ...
    try {
        ...
        for (const auto& tensor : pack.tensors()) {
            if (tensor.ggml_type == 12 && tensor.stored_bytes >= q38::kQ4KBytesPerBlock) {
                target = &tensor;
                break;
            }
        }
        if (!target) {
            std::cerr << "q38-q4k-smoke: no GGML_TYPE_Q4_K (12) tensor found\n";
            return 3;
        }

        std::cout << "Q38RT real-model Q4_K dequant smoke\n";
        ...
        if (!driver.run_q4k_dequant_smoke(block, &error, &max_abs, &max_rel)) {
            std::cerr << "q4_k: FAILED: " << error << "\n";
            return 4;
        }
