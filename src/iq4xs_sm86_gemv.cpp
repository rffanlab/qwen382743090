...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--model" && i + 1 < argc) model = argv[++i];
            ...
            std::cout
                << "usage: q38-iq4xs-sm86-gemv --model MODEL.q38pack [--tensor NAME]\n";
            return 0;
        } else {
    ...
    if (model.empty()) {
        std::cerr << "q38-iq4xs-sm86-gemv: --model is required\n";
        return 2;
    }
    ...
    try {
        ...
        if (!tensor) {
            std::cerr
                << "q38-iq4xs-sm86-gemv: tensor not found: "
                << tensor_name << "\n";
            return 3;
        }
        if (tensor->ggml_type != 23 ||
            ...
            std::cerr
                << "q38-iq4xs-sm86-gemv: tensor must be IQ4_XS/GGUF_NATIVE, type="
                << tensor->ggml_type
                << " layout="
        ...
        if (tensor->ndim < 2 ||
            ...
            tensor->dims[1] == 0) {
            std::cerr
                << "q38-iq4xs-sm86-gemv: tensor is not a 2-D matrix\n";
            return 5;
        }
        ...
        if (cols % q38::kQ4KValuesPerBlock != 0) {
            std::cerr
                << "q38-iq4xs-sm86-gemv: cols must be divisible by 256\n";
            return 6;
        }

        const std::size_t blocks_per_row =
            cols / q38::kQ4KValuesPerBlock;
        const std::size_t total_blocks =
            static_cast<std::size_t>(rows) * blocks_per_row;
        const std::size_t original_bytes =
            total_blocks * q38::kIQ4XSBytesPerBlock;

        if (tensor->stored_bytes < original_bytes) {
            std::cerr
                << "q38-iq4xs-sm86-gemv: tensor storage is smaller than expected IQ4_XS payload\n";
            return 7;
        }

        const std::size_t d_bytes = total_blocks * 2;
        const std::size_t scale_offset =
            align_up(d_bytes, 128);
        const std::size_t scale_bytes =
            total_blocks * 8;
        const std::size_t qs_offset =
            align_up(scale_offset + scale_bytes, 128);
        const std::size_t qs_bytes =
            total_blocks * 128;
        const std::size_t repacked_bytes =
            qs_offset + qs_bytes;

        std::vector<std::byte> repacked(repacked_bytes);
        std::array<std::byte, q38::kIQ4XSSm86BytesPerBlock> temp{};

        const auto* source = pack.tensor_data(*tensor);

        const auto repack_t0 =
            std::chrono::steady_clock::now();
        for (std::size_t ib = 0; ib < total_blocks; ++ib) {
            q38::repack_iq4_xs_sm86_block(
                source + ib * q38::kIQ4XSBytesPerBlock,
                temp.data());
            ...
            std::memcpy(
                ...
                2);
            std::memcpy(
                repacked.data() + scale_offset + ib * 8,
                temp.data() + 2,
                8);
            std::memcpy(
                repacked.data() + qs_offset + ib * 128,
                temp.data() + 10,
                128);
        }
        const auto repack_t1 =
            std::chrono::steady_clock::now();
        const double repack_ms =
            std::chrono::duration<double, std::milli>(
                repack_t1 - repack_t0).count();
        ...
        double repack_decode_max_abs = 0.0;
        {
            ...
            std::array<std::byte, q38::kIQ4XSSm86BytesPerBlock> check_block{};

            const std::size_t checked_blocks =
                std::min<std::size_t>(total_blocks, 64);

            for (std::size_t ib = 0; ib < checked_blocks; ++ib) {
                q38::dequantize_iq4_xs_block_cpu(
                    source + ib * q38::kIQ4XSBytesPerBlock,
                    original);
                ...
                q38::dequantize_iq4_xs_sm86_block_cpu(
                    check_block.data(),
                    transformed);

                for (std::size_t j = 0;
                     j < q38::kQ4KValuesPerBlock; ++j) {
                    repack_decode_max_abs =
                        std::max(
                            repack_decode_max_abs,
                            std::abs(
                                static_cast<double>(original[j]) -
                                static_cast<double>(transformed[j])));
                }
            }

            if (repack_decode_max_abs != 0.0) {
                std::cerr
                    << "q38-iq4xs-sm86-gemv: repack changed decoded weights, max_abs="
                    << repack_decode_max_abs << "\n";
                return 8;
            }
        }

        std::cout << "Q38RT SM86-repacked IQ4_XS SoA GEMV\n";
        std::cout << "tensor: " << tensor->name << "\n";
        std::cout << "shape: [" << cols << "," << rows << "]\n";
        std::cout << "original_IQ4XS_bytes: " << original_bytes << "\n";
        std::cout << "repacked_bytes: " << repacked_bytes << "\n";
        std::cout << std::fixed << std::setprecision(3);
        std::cout
            << "repack_size_overhead_percent: "
            << (100.0 *
                static_cast<double>(repacked_bytes - original_bytes) /
                static_cast<double>(original_bytes))
            << "\n";
        std::cout << "offline_repack_ms: " << repack_ms << "\n";
        std::cout << std::scientific << std::setprecision(6);
        std::cout
            << "repack_decode_max_abs_error: "
            << repack_decode_max_abs << "\n";
        std::cout << std::fixed << std::setprecision(3);
        std::cout
            << "layout: D2_SOA + SCALE8_SOA + QS128_SOA\n";
        ...
        if (!driver.run_iq4xs_sm86_gemv_smoke(
                repacked.data(),
                scale_offset,
                qs_offset,
                cols,
