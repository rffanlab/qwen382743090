...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--model" && i + 1 < argc) model = argv[++i];
            ...
            std::cout << "usage: q38-q5k-q8k-gemv --model MODEL.q38pack [--tensor NAME]\n";
            return 0;
        } else {
    ...
    if (model.empty()) {
        std::cerr << "q38-q5k-q8k-gemv: --model is required\n";
        return 2;
    }
    ...
    try {
        ...
        if (tensor->ndim < 2 || tensor->dims[0] == 0 || tensor->dims[1] == 0) {
            std::cerr << "q38-q5k-q8k-gemv: tensor is not a 2-D matrix\n";
            return 5;
        }
        ...
        if (!driver.run_q5k_q8k_gemv_smoke(
                ...
                &ms,
                &gbps)) {
            std::cerr << "q5_k_x_q8_k: FAILED: " << error << "\n";
            return 6;
        }

        std::cout << std::scientific << std::setprecision(6);
        std::cout << "max_abs_error_vs_q8_reference: " << max_abs << "\n";
        std::cout << "max_rel_error_vs_q8_reference: " << max_rel << "\n";
        ...
        std::cerr << "q38-q5k-q8k-gemv: " << e.what() << "\n";
        return 1;
    }
}
