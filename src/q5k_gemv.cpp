...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--model" && i + 1 < argc) model = argv[++i];
            ...
            std::cout << "usage: q38-q5k-gemv --model MODEL.q38pack [--tensor NAME]\n";
            return 0;
        } else {
    ...
    if (model.empty()) {
        std::cerr << "q38-q5k-gemv: --model is required\n";
        return 2;
    }
    ...
    try {
        ...
        if (!tensor) {
            std::cerr << "q38-q5k-gemv: tensor not found: " << tensor_name << "\n";
            return 3;
        }
        if (tensor->layout != q38::TensorLayout::GgufNative) {
            std::cerr << "q38-q5k-gemv: this baseline requires GGUF_NATIVE Q5_K; "
                      << "use q38-q5k-sm86-gemv for SM86_Q5K_SOA\n";
            return 11;
        }
        if (tensor->ggml_type != 13) {
            std::cerr << "q38-q5k-gemv: tensor is not GGML_TYPE_Q5_K (13), type="
                      << tensor->ggml_type << "\n";
            return 4;
        }
        if (tensor->ndim < 2 || tensor->dims[0] == 0 || tensor->dims[1] == 0) {
            std::cerr << "q38-q5k-gemv: tensor is not a 2-D matrix\n";
            return 5;
        }
        ...
        if (!driver.run_q5k_gemv_smoke(
                ...
                &ms,
                &gbps)) {
            std::cerr << "q5_k_gemv: FAILED: " << error << "\n";
            return 6;
        }
        ...
        std::cout << "q5_k_gemv: PASS\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "q38-q5k-gemv: " << e.what() << "\n";
        return 1;
    }
}
